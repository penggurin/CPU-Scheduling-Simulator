...
const pastelColors = [
    { bg: 'bg-pink-300', dark: 'bg-pink-400', hex: '#f9a8d4', darkHex: '#f472b6' },
    { bg: 'bg-purple-300', dark: 'bg-purple-400', hex: '#d8b4fe', darkHex: '#c084fc' },
    { bg: 'bg-blue-300', dark: 'bg-blue-400', hex: '#93c5fd', darkHex: '#60a5fa' },
    { bg: 'bg-green-300', dark: 'bg-green-400', hex: '#86efac', darkHex: '#4ade80' },
    { bg: 'bg-yellow-300', dark: 'bg-yellow-400', hex: '#fde047', darkHex: '#facc15' },
    { bg: 'bg-orange-300', dark: 'bg-orange-400', hex: '#fdba74', darkHex: '#fb923c' },
    { bg: 'bg-red-300', dark: 'bg-red-400', hex: '#fca5a5', darkHex: '#f87171' },
    { bg: 'bg-indigo-300', dark: 'bg-indigo-400', hex: '#a5b4fc', darkHex: '#818cf8' },
];
...
function toggleDarkMode() {
    ...
    if (darkMode) {
        ...
        body.classList.remove('bg-gradient-to-br', 'from-purple-50', 'via-pink-50', 'to-blue-50');
        ...
        body.classList.add('bg-gradient-to-br', 'from-purple-50', 'via-pink-50', 'to-blue-50');
...
function updateDarkModeStyles(isDark) {
    const elements = {
        ...
        labels: ['algoLabel', 'numJobsLabel', 'timeQuantumLabel'],
        inputs: ['algorithm', 'numJobs', 'timeQuantum'],
    ...
    if (isDark) {
        ...
        toggle.classList.remove('bg-gray-700', 'border-gray-600', 'hover:bg-gray-600');
    }

    const resetButton = document.getElementById('resetBtn');
    if (isDark) {
        resetButton.classList.remove('border-gray-300', 'hover:bg-gray-100', 'text-gray-700');
        resetButton.classList.add('border-gray-600', 'hover:bg-gray-700', 'text-gray-200', 'bg-gray-800');
    } else {
        resetButton.classList.add('border-gray-300', 'hover:bg-gray-100', 'text-gray-700');
        resetButton.classList.remove('border-gray-600', 'hover:bg-gray-700', 'text-gray-200', 'bg-gray-800');
    }
...
function renderJobsTable() {
    ...
    jobs.forEach((job, index) => {
        ...
        row.className = darkMode ? 'border-b border-gray-700' : 'border-b border-gray-100';
        row.innerHTML = `
...
function calculateSJF() {
    const gantt = [];
    const jobResults = [];
    const remainingJobs = jobs.map(j => ({ ...j }));
    let currentTime = 0;
    let completed = 0;

    while (completed < jobs.length) {
        const availableJobs = remainingJobs.filter(
            j => j.arrivalTime <= currentTime && j.burstTime > 0
        );
...
function calculatePriority() {
    ...
    while (completed < jobs.length) {
        ...
        jobResults.push({ jobId: highestPriorityJob.id, turnaroundTime, waitingTime });

        currentTime = endTime;
        highestPriorityJob.burstTime = 0;
        completed++;
    }
...
function calculateRoundRobin() {
    ...
    let currentTime = 0;
    let completed = 0;
    const visited = jobs.map(() => false);

    if (jobs.length > 0 && jobs[0].arrivalTime <= 0) {
        queue.push(0);
        visited[0] = true;
    }
    ...
    while (completed < jobs.length) {
        if (queue.length === 0) {
            for (let i = 0; i < jobs.length; i++) {
                if (!visited[i] && jobs[i].arrivalTime <= currentTime) {
                    queue.push(i);
                    visited[i] = true;
                }
            }
        ...
        const startTime = currentTime;
        const executeTime = Math.min(timeQuantum, remainingBurst[index]);
    ...
    const jobResults = jobs.map((job, index) => {
        const completionTime = gantt
            .filter(g => g.jobId === job.id)
            .reduce((max, g) => Math.max(max, g.endTime), 0);
...
function renderGanttChart(gantt, maxTime) {
    ...
    gantt.forEach((item, index) => {
        ...
        bar.className = 'gantt-bar flex items-center justify-center font-semibold border-2';
...
function renderStatistics(results) {
    ...
    avgRow.innerHTML = `
        ...
        <td class="py-2 px-4 ${darkMode ? 'text-gray-200' : 'text-gray-700'}">${avgTurnaround}</td>
        <td class="py-2 px-4 ${darkMode ? 'text-gray-200' : 'text-gray-700'}">${avgWaiting}</td>
    `;
